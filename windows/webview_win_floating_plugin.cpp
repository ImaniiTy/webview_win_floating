...
void WebviewWinFloatingPlugin::RegisterWithRegistrar(
  ...
  g_NativeHWND =
      GetAncestor(registrar->GetView()->GetNativeWindow(), GA_ROOT); // Jacky
  gMethodChannel = new flutter::MethodChannel<flutter::EncodableValue>(
      registrar->messenger(), "webview_win_floating",
...
void WebviewWinFloatingPlugin::HandleMethodCall(
  ...
  if (method_call.method_name().compare("initPlugin") == 0) {
    for (auto iter = webviewMap.begin(); iter != webviewMap.end(); iter++) {
      std::cout << "[webview_win_floating] old webview found, deleting"
                << std::endl;
      delete iter->second;
    }
    webviewMap.clear();
  ...
  if (isCreateCall) {
    ...
    auto onCreate = [shared_result, webviewId,
                     url](HRESULT hr, MyWebView *webview) -> void {
      if (webview != NULL) {
        webviewMap[webviewId] = webview;
        std::cout << "[webview] native create: id = " << webviewId << std::endl;
        if (!url.empty())
          webview->loadUrl(toWideString(url));
    ...
    auto onPageStarted = [webviewId](std::string url, bool isNewWindow,
                                     bool isUserInitiated) -> void {
      flutter::EncodableMap arguments;
      ...
      arguments[flutter::EncodableValue("isUserInitiated")] =
    ...
    if (!userDataFolder.empty()) {
      auto convResult = MultiByteToWideChar(
          CP_UTF8, 0, userDataFolder.c_str(), -1, wUserDataFolder,
          sizeof(wUserDataFolder) / sizeof(WCHAR));
      if (convResult < 0) {
        std::cout << "[webview_win_floating] native convert userDataFolder to "
                     "utf16 (WCHAR*) failed: path = "
                  << userDataFolder << std::endl;
      } else {
    ...
    auto hr = webview->runJavascript(
        toWideString(javaScriptString), ignoreResult,
    ...
    if (FAILED(hr))
      result->Error("runJavascript() error");
  ...
  } else if (method_call.method_name().compare("addCookie") == 0) {
    auto name =
        std::get<std::string>(arguments[flutter::EncodableValue("name")]);
    auto value =
        std::get<std::string>(arguments[flutter::EncodableValue("value")]);
    auto domain =
        std::get<std::string>(arguments[flutter::EncodableValue("domain")]);
    auto path =
        std::get<std::string>(arguments[flutter::EncodableValue("path")]);

    webview->addCookie(toWideString(name), toWideString(value),
                       toWideString(domain), toWideString(path));
