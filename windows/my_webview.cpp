...
std::string Utf8FromUtf16(LPWSTR wstr) {
  DWORD dBufSize =
      WideCharToMultiByte(CP_OEMCP, 0, wstr, -1, NULL, 0, NULL, FALSE);
  char *dBuf = new char[dBufSize];
  memset(dBuf, 0, dBufSize);
  int nRet =
      WideCharToMultiByte(CP_OEMCP, 0, wstr, -1, dBuf, dBufSize, NULL, FALSE);
  if (nRet <= 0)
    return "";
...
public:
  MyWebViewImpl(HWND hWnd, std::function<void(HRESULT, MyWebView *)> onCreated,
                std::function<void(std::string url, bool isNewWindow,
  ...
  HRESULT runJavascript(PCWSTR javaScriptString, bool ignoreResult,
  ...
  HRESULT addCookie(LPCWSTR name, LPCWSTR value, LPCWSTR domain, LPCWSTR path);
...
private:
  ...
  std::map<std::wstring, std::wstring>
      channelMap; // channel name -> id of
                  // RemoveScriptToExecuteOnDocumentCreated
  std::map<std::wstring, std::wstring>
      headers; // channel name -> id of RemoveScriptToExecuteOnDocumentCreated
...
MyWebView *MyWebView::Create(
    HWND hWnd, std::function<void(HRESULT, MyWebView *)> callback,
    std::function<void(std::string url, bool isNewWindow, bool isUserInitiated)>
        onPageStarted,
    std::function<void(std::string, int errCode)> onPageFinished,
    std::function<void(std::string)> onPageTitleChanged,
    std::function<void(std::string)> onWebMessageReceived,
...
HRESULT InitWebViewRuntime(PCWSTR pwUserDataFolder,
                           std::function<void(HRESULT)> callback = nullptr) {
  if (g_env != NULL) {
    if (callback != nullptr)
      callback(S_OK);
    return S_OK;
  ...
  return CreateCoreWebView2EnvironmentWithOptions(
      nullptr, pwUserDataFolder, nullptr,
...
HRESULT ReleaseWebViewRuntime() { return S_OK; }

MyWebViewImpl::MyWebViewImpl(
    HWND hWnd, std::function<void(HRESULT, MyWebView *)> onCreated,
    std::function<void(std::string url, bool isNewWindow, bool isUserInitiated)>
        onPageStarted,
    std::function<void(std::string, int errCode)> onPageFinished,
    std::function<void(std::string)> onPageTitleChanged,
    ...
    g_env->CreateCoreWebView2Controller(
        ...
        Callback<ICoreWebView2CreateCoreWebView2ControllerCompletedHandler>(
            [=](HRESULT hr, ICoreWebView2Controller *controller) -> HRESULT {
              ...
              addCookie(L"xf_session", L"zDzQjCtRLTr0Xd0DuQKatAH1i1_F4HZ1",
                        L"f95zone.to", L"/");
                      ...
                      [=](ICoreWebView2 *sender,
                        ...
                        if (SUCCEEDED(hr)) {
                          ...
                          if (m_isNowGoBackForward ||
                            ...
                            // is triggered by loadUrl() or loadHtmlString(),
                            // not user initiated or is triggered by goBack /
                            // goForward then we don't ask client Dart code
                            // (onNavigationRequest) to allow/prevent loading
                            // url
                            nowLoadingUrl = L"";
                          ...
                          // always cancel user initiated navigation, and pass
                          // this event to [webview_flutter] and after
                          // [webview_flutter] ask client dart code, if client
                          // say yes, [webview_flutter] then call loadUrl() to
                          // load url again
                          if (userInitiated)
                  ...
                  Callback<ICoreWebView2WebResourceRequestedEventHandler>(
                      [=](ICoreWebView2 *sender,
                        ...
                        for (auto const &p : headers) {
                          requestHeaders->SetHeader(p.first.c_str(),
                                                    p.second.c_str());
                        }
                  ...
                  Callback<ICoreWebView2NewWindowRequestedEventHandler>(
                      [=](ICoreWebView2 *sender,
                        ...
                        if (SUCCEEDED(hr)) {
                          ...
                          // bool isAllowed = m_bAllowNewWindow &&
                          // checkUrlAllowed(utf8Url); //TODO
                  ...
                  Callback<ICoreWebView2NavigationCompletedEventHandler>(
                      [=](ICoreWebView2 *sender,
                        ...
                        if (!success) {
                          ...
                          if (SUCCEEDED(hr)) {
                            errCode = webErrorStatus; // TODO: enum all the
                                                      // error code...
                          }
                        }
              ...
              hr = m_pWebview->add_WebMessageReceived(
                  Callback<ICoreWebView2WebMessageReceivedEventHandler>(
                      ...
                      .Get(),
                  NULL); /// &m_webMessageReceivedToken
              ...
              hr = m_pWebview->add_ContainsFullScreenElementChanged(
                  Callback<
                      ...
                      [=](ICoreWebView2 *sender, IUnknown *args) -> HRESULT {
                        BOOL isFullScreen;
                        m_pWebview->get_ContainsFullScreenElement(
                            &isFullScreen);
...
HRESULT
MyWebViewImpl::runJavascript(LPCWSTR javaScriptString, bool ignoreResult,
                             std::function<void(std::string)> callback) {
  return m_pWebview->ExecuteScript(
      javaScriptString,
      Callback<ICoreWebView2ExecuteScriptCompletedHandler>(
          [callback, ignoreResult](HRESULT hr,
                                   LPCWSTR resultObjectAsJson) -> HRESULT {
            if (callback != nullptr) {
              if (ignoreResult)
                callback("");
              else
                callback(utf8_encode(resultObjectAsJson));
            }
            return hr;
          })
          .Get());
...
HRESULT MyWebViewImpl::addScriptChannelByName(LPCWSTR channelName) {
  if (!m_hasRegisteredChannel) {
    ...
    LPCWSTR script = L"class JkChannel { constructor(name) { this.name = name; "
                     L"} postMessage(message) { "
                     L"window.chrome.webview.postMessage({'JkChannelName': "
                     L"this.name, 'msg' : message}); } }";
  ...
  WCHAR script[100];
  if (wcslen(channelName) > 30)
    return E_FAIL;
  wsprintf(script, L"const %s = new JkChannel('%s');", channelName,
           channelName);
...
HRESULT MyWebViewImpl::setBackgroundColor(int32_t argb) {
  ...
  wil::com_ptr<ICoreWebView2Controller2> controller2 =
      m_pController.query<ICoreWebView2Controller2>();
  return controller2->put_DefaultBackgroundColor(value);
}

HRESULT MyWebViewImpl::requestFocus(bool isNext) {
  m_pController->MoveFocus(isNext ? COREWEBVIEW2_MOVE_FOCUS_REASON_NEXT
...
void MyWebViewImpl::reload() { m_pWebview->Reload(); }

void MyWebViewImpl::cancelNavigate() { m_pWebview->Stop(); }

template <class T> wil::com_ptr<T> MyWebViewImpl::getProfile() {
  static_assert(std::is_base_of<ICoreWebView2Profile, T>::value,
                "T must inherit from <ICoreWebView2Profile>");
...
HRESULT MyWebViewImpl::addCookie(LPCWSTR name, LPCWSTR value, LPCWSTR domain,
                                 LPCWSTR path) {
  wil::com_ptr<ICoreWebView2CookieManager> cookieManager;
  auto webview2_2 = m_pWebview.try_query<ICoreWebView2_2>();
  if (webview2_2 == NULL)
  ...
  cookieManager->CreateCookie(name, value, domain, path, &cookie);
...
HRESULT MyWebViewImpl::suspend() {
  ...
  if (webview2_3 == NULL)
    return E_FAIL;
  return webview2_3->TrySuspend(
      Callback<ICoreWebView2TrySuspendCompletedHandler>(
          [=](HRESULT errorCode, BOOL isSuccessful) -> HRESULT { return S_OK; })
          .Get());
}
...
void MyWebViewImpl::openDevTools() { m_pWebview->OpenDevToolsWindow(); }